...
from .dataset_readers import readCamerasWithPriorsFromTxt
...
class SRNPriorsDataset(SharedDataset):
    def __init__(self, cfg, dataset_root, dataset_name="train"):
        ...
        self.base_path = os.path.join(dataset_root, "srn_{}_priors/{}_{}".format(cfg.data.category,
                                                                                   cfg.data.category,
                                                                                   self.dataset_name))
    ...
    def load_example_id(self, example_id, intrin_path,
        ...
        normal_paths = sorted(glob.glob(os.path.join(dir_path, "normal", "*")))
        ...
        if example_id not in self.all_rgbs.keys():
            ...
            cam_infos = readCamerasWithPriorsFromTxt(rgb_paths, depth_paths, normal_paths, pose_paths, [i for i in range(len(rgb_paths))])

            for cam_info in cam_infos:
                R = cam_info.R
                T = cam_info.T

                self.all_rgbs[example_id].append(PILtoTorch(cam_info.rgb_image, 
                                                            (self.cfg.data.training_resolution, self.cfg.data.training_resolution)).clamp(0.0, 1.0)[:3, :, :])
                self.all_depths[example_id].append(PILtoTorch(cam_info.depth_image, 
                                                            (self.cfg.data.training_resolution, self.cfg.data.training_resolution)).clamp(0.0, 1.0)[:3, :, :])
                self.all_normals[example_id].append(PILtoTorch(cam_info.normal_image, 
                                                            (self.cfg.data.training_resolution, self.cfg.data.training_resolution)).clamp(0.0, 1.0)[:3, :, :])
            ...
            self.all_normals[example_id] = torch.stack(self.all_normals[example_id])
    ...
    def __getitem__(self, index):
        ...
        images_and_camera_poses = {
            ...
            "pred_depths": self.all_depths[example_id][frame_idxs].clone(),
            "pred_normals": self.all_normals[example_id][frame_idxs].clone(),
