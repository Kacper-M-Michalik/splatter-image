...
class SRNPriorsDataset(SharedDataset):
    def __init__(self, cfg, dataset_name="train"):
        ...
        self.dataset_intrins = self.dataset_intrins.sort('uuid')
    ...
    def load_example_id(self, intrin_idx, trans = np.array([0.0, 0.0, 0.0]), scale=1.0):
        uuid = self.dataset_intrins[intrin_idx]
        ...
        if uuid not in self.all_rgbs.keys():
            ...
            filter_uuid = lambda data: data['uuid'] == uuid and data['split'] == self.dataset_name
            print(len(self.dataset_poses.filter(filter_uuid).sort('frame_id')))
            cam_infos = readCamerasWithPriorsFromHF(uuid, 
                                                    self.dataset_poses.filter(filter_uuid).sort('frame_id'),
                                                    self.dataset_rgbs.filter(filter_uuid).sort('frame_id'), 
                                                    self.dataset_depths.filter(filter_uuid).sort('frame_id'), 
                                                    self.dataset_normals.filter(filter_uuid).sort('frame_id'))
            ...
            for cam_info in cam_infos:
                ...
                image = (cam_info.rgb_image / 255).clamp(0.0, 1.0)
                print(image.shape)
                ...
                image = (cam_info.depth_image / 255).clamp(0.0, 1.0)
                print(image.shape)
                ...
                image = (cam_info.normal_image / 255).clamp(0.0, 1.0)
                print(image.shape)
